...
class ImageProcessorWorker(QObject):
    """Worker object to process images in a separate thread"""
    progress = Signal(int, int)  # current, total
    finished = Signal(list)      # processed images
    ...
    def process_images(self):
        ...
        try:
            # Scan for images
            image_files = self.file_manager.scan_directory(self.directory)
            ...
            # Emit finished signal with processed images
...
class MainWindow(QMainWindow):
    def __init__(self, db_manager, file_manager):
        ...
        self.photo_viewer.close_signal.connect(self.close_single_view)
        self.photo_viewer.next_signal.connect(self.next_photo)
        self.photo_viewer.prev_signal.connect(self.prev_photo)
    ...
    def open_folder(self):
        """Open a folder dialog and scan for images in a background thread"""
        ...
        if folder:
            ...
            self.loading_overlay.set_message(f"Loading images from {folder}")
    ...
    def on_processing_finished(self, processed_images):
        ...
        self.statusBar().showMessage(f"Loaded {num_images} images")
    ...
    def on_photo_selected(self, photo_data):
        """Handle photo selection (single click) - just updates UI state"""
        # Update status bar with photo info
        file_name = photo_data['file_path'].split('/')[-1].split('\\')[-1]
        size_kb = photo_data.get('file_size', 0) / 1024
        self.statusBar().showMessage(f"Selected: {file_name} ({size_kb:.1f} KB)")
    ...
    def refresh_photos(self):
        ...
        photos = self.db_manager.get_all_photos(sort_by=sort_by, sort_order=sort_order)
        self.photo_grid.load_photos(photos)
        self.statusBar().showMessage(f"Displaying {len(photos)} photos")
    ...
    def open_single_view(self, photo_data):
        """Open the single photo view with the selected photo"""
        self.photo_grid.hide()
        self.photo_viewer.load_photo(photo_data)
    ...
    def close_single_view(self):
        """Close the single photo view and return to grid"""
        self.photo_viewer.hide()
        self.photo_grid.show()
        self.view_mode_combo.setCurrentIndex(0)  # Update the view mode combo

    def next_photo(self):
        """Show the next photo in single view"""
        next_photo = self.photo_grid.get_next_photo()
        if next_photo:
            self.photo_viewer.load_photo(next_photo)

    def prev_photo(self):
        """Show the previous photo in single view"""
        prev_photo = self.photo_grid.get_prev_photo()
        if prev_photo:
            self.photo_viewer.load_photo(prev_photo)
